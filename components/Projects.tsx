  ...
  return (
    <section id="work" className="mx-auto max-w-shell px-6 py-24 md:px-10">
      ...
      <div className="mt-10 grid gap-6 md:grid-cols-2">
        {projects.map((p, i) => (
          <Parallax key={p.title} speed={i % 2 === 0 ? 0.18 : 0.42} className="h-full">
                ...
                <div className="flex flex-1 flex-col gap-3.5 px-7 pb-7 pt-5">
                  ...
                  {(p.url || p.github) && (
                    <div className="flex gap-4 pt-1 text-sm font-semibold">
                      {p.url && (
                        <a href={p.url} target="_blank" rel="noreferrer" className="text-cyan hover:underline">
                          Live ↗
                        </a>
                      )}
                      {p.github && (
                        <a href={p.github} target="_blank" rel="noreferrer" className="text-muted hover:text-ink">
                          GitHub ↗
                        </a>
                      )}
                    </div>
